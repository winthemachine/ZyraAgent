...
class EthTopTradersService:
    ...
    def fetch_top_traders(self, contract_address: str):
        """Fetch top traders data for a single ETH contract address"""
        url = f"https://gmgn.ai/defi/quotation/v1/tokens/top_traders/eth/{contract_address}?orderby=profit&direction=desc"
        retries = 3
            ...
            try:
                ...
                if multiplier_value:
                    processed_trader = {
                        ...
                        "win_rate": f"{trader.get('win_rate', 0) * 100:.2f}%",
                        "avg_hold_time": trader.get('avg_hold_time', 0),
                        "last_trade": trader.get('last_trade_time', 0)
                    }
    ...
    def get_eth_top_traders(self, contract_addresses, max_threads=5):
        """Get top traders for multiple ETH contract addresses"""
        ...
        with ThreadPoolExecutor(max_workers=max_threads) as executor:
            futures = {
                executor.submit(self.fetch_top_traders, address): address 
